...
.write()`方法的<a href="https://github.com/nodejs/node/blob/55c42bc6e5602e5a47fb774009cfe9289cb88e71/lib/_stream_writable.js#L239" fo="25">返回值</a>，我们得到以下结果：
...
这个有效的举措允许一大堆锁住的内存可以为[`.pipe()`][]函数随时使用而并没有内存泄露、无限扩张的内存缓冲。同时垃圾回收器仅需要处理一处地方。
...
[方法，我们同样要小心在 <3>`._read()`]: https://nodejs.org/docs/latest/api/stream.html#stream_readable_read_size_1
