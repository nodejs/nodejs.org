...
Lastly, `process.stdin` is a readable stream for getting user input.  See [more on cli input](/uk/knowledge/command-line/how-to-prompt-for-command-line-input).
...
The `process` module also exposes `process.argv`, an array containing the command-line arguments to the current process, and `process.argc`, an integer representing the number of arguments passed in.  Read more on [how to parse command line arguments](/uk/knowledge/command-line/how-to-parse-command-line-arguments)
