...
var url = request.url;
```
> **Note:** `request` 객체는 [`IncomingMessage`][]의 인스턴스입니다.
...
어떤 경우에는 이게 문제가 될 수도 있으므로 [`rawHeaders`][]를 사용할 수도 있습니다.
...
> **Note:** 이 코드가 약간 장황할 수도 있고 대부분은 실제로 그렇습니다. 다행히 [`npm`][]에
[`concat-stream`][]나 [`body`][]같은 모듈로 이 로직을 감출 수 있습니다. 이어서 읽기 전에
어떤 작업이 이뤄지는지 잘 이해하는 것이 중요합니다. 이 때문에 이 글을 읽고 있는 것입니다.
...
설정이 귀찮지 않도록 응답의 HTTP 상태 코드는 항상 200입니다. 물론 모든 HTTP 응답이 이를 보장하는
...
> **Note:** 바디에 데이터 청크를 *작성하기 전에* 상태 코드와 헤더를 설정해야 합니다.
...
이제 HTTP 응답을 만드는 것에 대해서 배웠습니다. 이제 이 모든 것을 함께 사용해 보겠습니다. 이전에 본 예제에서 사용자가 서버에 보낸 모든 데이터를 다시 보내는 서버를 만들 것입니다. `JSON.stringify`를 사용해서 데이터를 JSON으로 포매팅할 것입니다.
...
http.createServer(function(request, response) {
  ...
  request.on('error', function(err) {
    ...
    response.statusCode = 200;
    response.setHeader('Content-Type', 'application/json');
    // Note: 위 두 줄은 다음 한 줄로 대체할 수도 있습니다.
    // response.writeHead(200, {'Content-Type': 'application/json'})
...
```
-->
> **Note:** 이 방법으로 URL을 검사함으로써 "라우팅"을 하고 있습니다. `switch` 문으로 간단히
