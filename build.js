...
function fullBuild (opts) {
  ...
  getSource((err, source) => {
    ...
    fs.readdir(path.join(__dirname, 'locale'), (e, locales) => {
      ...
      const asyncTasks = filteredLocales.map((locale) => (
        buildLocale(source, locale, { preserveLocale, localesData })
      ))

      Promise.all(asyncTasks)
        .then(
          _ => console.info('\x1B[32mAll build successful!\x1B[39m'),
          err => console.error('\x1B[31mBuild Failed, The error is: %s\x1B[0m', err))
    })
  })
}
