...
```bash
turbo install --frozen-lockfile # installs this project's dependencies
...
8. Run `turbo format` to confirm that linting, and formatting are passing.
...
- `pnpm export` exports the website from the `.next` into a fully static website. The output is by default within `build` folder.
  - This is what it's used to deploy the website on our current Node.js servers.
- `pnpm start` starts a web server running serving the built content from `npm run build`
...
- `turbo storybook` starts Storybook's local server
- `turbo build-storybook` builds Storybook as a static web application for publishing
