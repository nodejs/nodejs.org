...
Dans l'exemple suivant, "hello world", de nombreuses connexions
peuvent être gérées simultanément. À chaque connexion, le rappel est déclenché
...
Les réseaux basés sur les threads sont relativement inefficaces et très
difficile à utiliser. En outre, les utilisateurs de Node.js n'ont pas à s'inquiéter de bloquer le processus, puisqu'il n'y a pas de verrous.
Presque aucune fonction de Node.js n'exécute directement des E/S, de sorte que le processus ne se bloque jamais, sauf lorsque les E/S sont exécutées à l'aide des 
...
La conception de Node.js est similaire et influencée par des systèmes tels que [Event Machine][] de Ruby et [Twisted][] de Python. 
Node.js pousse le modèle d'événement un peu plus loin.
Il présente une boucle d'événements comme une construction d'exécution plutôt que comme une bibliothèque. Dans d'autres systèmes,
...
Dans Node.js, il n'y a pas de tel appel demmarer-la-boucle. Node.js entre simplement dans la boucle d'événements après avoir exécuté le script d'entrée. Node.js
...
HTTP est un citoyen de première classe dans Node.js, conçu pour la diffusion en continu et la faible latence. Node.js est donc bien adapté à la création d'une bibliothèque web ou d'un framework.

Le fait que Node.js soit conçu sans threads ne signifie pas que vous ne pouvez pas profiter de plusieurs cœurs dans votre environnement. 
Les processus fils peuvent être créés en utilisant notre API [`child_process.fork()`][], et sont conçus pour faciliter la communication avec eux.
Le module [`cluster`][] est construit sur cette même interface,
