...
- 이 취약점에 대한 제안된 금지 날짜가 선택되며, 이 취약점에 대한 CVE(공통 취약점 및 노출(CVE®))가 요청됩니다.

- 금지 날짜에 Node.js 보안 메일링 리스트에 발표 사본이 발송됩니다. 변경 사항이 공개 리포지토리에 푸시되고, 새로운 빌드가 nodejs.org에 배포됩니다. 메일링 리스트에 통지된 후 6시간 이내에 advisory 사본이 Node.js 블로그에 게시됩니다.

- 일반적으로 금지 날짜는 CVE가 발급된 시점으로부터 72시간으로 설정됩니다. 그러나 버그의 심각성이나 수정 적용의 어려움에 따라 다를 수 있습니다.

- 이 과정은 시간이 걸릴 수 있으며, 특히 다른 프로젝트의 유지보수자와의 조정이 필요할 경우 더욱 그러합니다. 버그를 최대한 신속하게 처리하기 위해 모든 노력을 기울이겠지만, 공개가 일관되게 처리되도록 위의 릴리스 프로세스를 따르는 것이 중요합니다.
...
오픈 소스 보안 재단(OpenSSF)의 [최선의 관행 배지](https://github.com/coreinfrastructure/best-practices-badge)는 자유/자유 소프트웨어(FLOSS) 프로젝트가 최선의 관행을 따르고 있음을 보여주는 방법입니다. 프로젝트는 각 최선의 관행을 따르는 방식을 자발적으로 자기 인증할 수 있습니다. 배지 소비자는 어떤 FLOSS 프로젝트가 최선의 관행을 따르고 있는지를 신속하게 평가할 수 있으며, 그 결과 더 높은 품질의 보안 소프트웨어를 생산할 가능성이 높아집니다.
